...
const BENTO_SPANS = ['bento-p-wide', '', 'bento-p-tall', '', '', 'bento-p-wide']
          ...
          <div className="bento-grid-portfolio">
            {displayItems.map((item, i) => (
              <motion.div
                ...
                className={`bento-card group ${BENTO_SPANS[i % BENTO_SPANS.length]}`}
              >
                {item.image_url ? (
                  <Image
                    src={item.image_url}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5">
                  <h3 className="text-white font-semibold text-sm sm:text-base mb-1 drop-shadow-sm">{item.title}</h3>
                  {item.description && (
                    <p className="text-white/60 text-xs line-clamp-2 leading-relaxed mb-2 group-hover:text-white/75 transition-colors">{item.description}</p>
                  )}
                  <div className="flex items-center gap-2">
                    {item.category && (
                      <span className="bento-pill-btn text-[10px] px-2.5 py-0.5">
                        {item.category}
                      </span>
                    )}
                    {item.client_name && (
                      <span className="text-[10px] text-white/40 group-hover:text-white/60 transition-colors">for {item.client_name}</span>
                    )}
                  </div>
                </div>
              </motion.div>
